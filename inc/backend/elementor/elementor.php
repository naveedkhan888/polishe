...
add_action( 'elementor/widgets/register', 'polishe_register_elementor_widgets' );
function polishe_register_elementor_widgets() {
...
// Add a custom 'category_polishe' category for to the Elementor element panel so that our theme's widgets have their own category.
add_action( 'elementor/init', function() {
    \Elementor\Plugin::$instance->elements_manager->add_category( 
        'category_polishe',
        [
            'title' => __( 'Polishe', 'polishe' ),
            'icon' => 'fa fa-plug', //default icon
        ],
        1 // position
    );
    \Elementor\Plugin::$instance->elements_manager->add_category( 
        'category_polishe_header',
        [
            'title' => __( 'XP Header', 'polishe' ),
            'icon' => 'fa fa-plug', //default icon
        ],
        2 // position
    );
...
function polishe_add_cpt_support() {
...
add_action( 'elementor/init', 'polishe_add_cpt_support' );

// Upload SVG for Elementor
function polishe_unfiltered_files_upload() {
...
add_action( 'elementor/init', 'polishe_unfiltered_files_upload' );



/*Fix Elementor Pro*/
function polishe_register_elementor_locations( $elementor_theme_manager ) {

    $elementor_theme_manager->register_all_core_location();

}
add_action( 'elementor/theme/register_locations', 'polishe_register_elementor_locations' );
...
add_action('elementor/element/container/section_layout/after_section_end', function( $container, $args ) {
    ...
    $container->add_control(
        'sticky_class',
        [
            'label'        => __( 'Sticky On/Off', 'polishe' ),
    ...
    $container->add_control(
        'sticky_background',
        [
            'label'     => __( 'Background Scroll', 'polishe' ),
...
add_action('elementor/element/container/section_layout/after_section_end', function( $container, $args ) {
    ...
    $container->start_controls_section(
        ...
        [
            'label' => __( 'Full Width Options', 'polishe' ),
            'tab'   => \Elementor\Controls_Manager::TAB_LAYOUT,
        ]
    );
    ...
    $container->add_control(
        'right_fullwidth',
        [
            'label'        => __( 'Right Full Width', 'polishe' ),
            'type'         => Elementor\Controls_Manager::SWITCHER,
    ...
    $container->add_control(
        'left_fullwidth',
        [
            'label'        => __( 'Left Full Width', 'polishe' ),
            'type'         => Elementor\Controls_Manager::SWITCHER,

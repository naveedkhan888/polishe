...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-post post-box'); ?>>
    ...
    <div class="inner-post <?php if( !has_post_thumbnail() ) echo 'no-ptop'; ?>">
        <?php if( $format != 'gallery' && $format != 'image' && $format != 'video' && !has_post_thumbnail() ) polishe_posted_in(); ?>
        <div class="entry-header">
            <div class="entry-meta">
                <?php if( polishe_get_option( 'post_entry_meta' ) ) { polishe_post_meta(); } ?>
            </div>
            <?php if( polishe_get_option( 'ptitle_post' ) ) the_title( '<h4 class="entry-title">', '</h4>' ); ?>
            ...
            <?php
                ...
                the_content(sprintf(
                    ...
                    /* translators: %s: Name of current post. Only visible to screen readers */
                        __('Continue reading<span class="screen-reader-text"> "%s"</span>', 'polishe'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
        ...
        <div class="entry-footer clearfix">
            <?php polishe_entry_footer(); ?>
        </div>
        <?php if( polishe_get_option('author_box') ) polishe_author_info_box(); ?>
        <?php if( polishe_get_option('post_nav') ) polishe_single_post_nav(); ?>
        <?php if( polishe_get_option('related_post') ) polishe_related_posts(); ?>
    </div>
